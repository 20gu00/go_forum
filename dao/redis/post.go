...
func GetPostIDsInOrder(p *param.ParamPostList) ([]string, error) {
	// 从redis获取id
	// 1. 根据用户请求中携带的order参数确定要查询的redis key
	key := getRedisKey(KeyPostTimeZSet)
	if p.Order == param.OrderScore {
...
func GetPostVoteData(ids []string) (data []int64, err error) {
	...
	// 使用pipeline一次发送多条命令,减少RTT(命令运行时间,网络连接延迟)
...
func GetCommunityPostIDsInOrder(p *param.ParamPostList) ([]string, error) {
